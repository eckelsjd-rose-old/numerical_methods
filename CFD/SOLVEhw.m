...
classdef SOLVE
    methods(Static)
        ...
        function [x, urt] = solve_burgers(N,nu,A,b,c)
            ...
            [~,x_mat,~] = SOLVE.generate_mesh(N,A,b,c);
            x = x_mat(:,1);
            % Define residual and Jacobian functions
            R = @(u)SOLVE.residual_burgers(u,x_mat,nu,umin,umax);
        ...
        function res = residual_burgers(u,x_mat,nu,umin,umax)
            N = size(u,1);      % number of discrete points
            x = x_mat(:,1);     % x(s)
            xp = x_mat(:,2);    % x'(s)
            xpp = x_mat(:,3);   % x''(s)
            ds = 1/(N-1);       % computational domain spacing (constant)
            res = zeros(N,1);
            
            for i = 2:N-1
                dudx = (1/2)*(((u(i+1)^2-u(i-1)^2)/(2*ds))*(1/xp(i)));
                duds = (u(i+1)-u(i-1))/(2*ds);
                du2ds2 = (u(i+1)-2*u(i)+u(i-1))/(ds^2);
                d2udx2 = du2ds2/(xp(i)^2) - duds*(xpp(i)/(xp(i)^3));
                res(i) = dudx - nu*d2udx2;
            end
